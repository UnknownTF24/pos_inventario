...
def init_db():
    ...
    # Tabla de Tiendas (Con ESTADO DE PAGO)
    cursor.execute("""
...
def verificar_token(authorization: str = Header(None)):
    ...
    try:
        ...
        return {
            "usuario": payload["sub"], "rol": payload.get("rol", "cajero"), 
            "nombre_completo": payload.get("nombre", payload["sub"]), "tienda_id": payload.get("tienda_id", 1)
        }
...
def login(req: LoginRequest):
    conn = get_db_connection()
    cursor = conn.cursor()
    # NUEVO: Revisamos el usuario y también el ESTADO de su tienda
    cursor.execute("""
        SELECT u.usuario, u.password, u.rol, u.nombre_completo, u.tienda_id, t.estado 
        FROM usuarios u
    ...
    if user:
        ...
        # REBOTAMOS AL CLIENTE SI NO HA PAGADO
...
def guardar_ajustes(req: dict, user_info: dict = Depends(verificar_token)):
    ...
    conn = get_db_connection(); cursor = conn.cursor()
    cursor.execute("UPDATE tiendas SET nombre=%s, direccion=%s, nit=%s, telefono=%s, mensaje_ticket=%s WHERE id=%s", (req.get("nombre"), req.get("direccion"), req.get("nit"), req.get("telefono"), req.get("footer"), user_info["tienda_id"]))
...
def historial_cajas(user_info: dict = Depends(verificar_token)):
    if user_info["rol"] not in ["superadmin", "admin"]: raise HTTPException(status_code=403, detail="Sin permisos.")
    conn = get_db_connection(); cursor = conn.cursor()
    cursor.execute("""SELECT c.id, c.cajero, c.fondo_inicial, c.fecha_apertura, c.fecha_cierre, c.estado, COALESCE((SELECT SUM(total) FROM ventas v WHERE v.cajero = c.cajero AND v.fecha >= c.fecha_apertura AND (c.fecha_cierre IS NULL OR v.fecha <= c.fecha_cierre) AND v.tienda_id = c.tienda_id), 0) as total_ventas FROM caja_sesiones c WHERE c.tienda_id = %s ORDER BY c.fecha_apertura DESC LIMIT 100""", (user_info["tienda_id"],))
...
def crear_usuario(req: UsuarioRequest, user_info: dict = Depends(verificar_token)):
    ...
    except Exception: conn.rollback(); raise HTTPException(status_code=400, detail="Usuario ya en uso."); 
    finally: conn.close()
